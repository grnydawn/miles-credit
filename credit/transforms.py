...
def load_transforms(conf, scaler_only=False):
    ...
    elif conf["data"]["scaler_type"] == "std_mpasa":
        transform_scaler = Normalize_MPASA_and_Forcing(conf)
    ...
    elif conf["data"]["scaler_type"] == "std_new" or conf["data"]["scaler_type"] == "std_cached":
        # std_new and std_cached ToTensor
        to_tensor_scaler = ToTensor_ERA5_and_Forcing(conf)

    elif conf["data"]["scaler_type"] == "std_mpasa":
        to_tensor_scaler = ToTensor_MPASA_and_Forcing(conf)
...
class Normalize_MPASA_and_Forcing:
...
class ToTensor_MPASA_and_Forcing:
    """Class to convert MPASA and Forcing Datasets to torch tensor."""
...
#                        numpy_vars_diagnostic = np.array(list_vars_diagnostic)
            ...
            elif key == "target_MPASA_images" or key == "y":
                if self.flag_surface:
