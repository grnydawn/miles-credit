...
class Diffusion_and_Pole_Filter:
    ...
    """
    def __init__(self, nlat, nlon, device='cpu', lmax=None, mmax=None, grid='legendre-gauss', radius=6.37122E6, omega=7.292E-5, gravity=9.80616, havg=10.e3, hamp=120.):
        """
    ...
    def initialize_sht(self):
        ...
        self.sht = harmonics.RealSHT(self.nlat, self.nlon, self.lmax, self.mmax, self.grid, csphase=False).to(self.device)
        self.isht = harmonics.InverseRealSHT(self.nlat, self.nlon, self.lmax, self.mmax, self.grid, csphase=False).to(self.device)
        self.vsht = harmonics.RealVectorSHT(self.nlat, self.nlon, self.lmax, self.mmax, self.grid, csphase=False).to(self.device)
        self.ivsht = harmonics.InverseRealVectorSHT(self.nlat, self.nlon, self.lmax, self.mmax, self.grid, csphase=False).to(self.device)
    ...
    def initialize_other_properties(self):
        ...
        self.invlap[0] = 0.  # Adjusting the first element to avoid division by zero
    ...
    def getgrad(self,chispec):
        ...
        ntrunc = int(-1.5 + 0.5*torch.sqrt(9.-8.*(1.-torch.tensor(self.spec2grid(chispec).shape[0]))))
