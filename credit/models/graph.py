...
class GraphResTransfGRU(BaseModel):
    def __init__(self, 
        ...
        self.graph_blocks = nn.ModuleList(
            GraphResidualBlock(self.hidden_size, self.hidden_size, self.hidden_size, self.heads, self.dropout, self.use_spectral_norm, self.use_edge_attr) for i in range(self.n_blocks)
        )
    ...
    def load_graph(self):
        ...
        if self.use_edge_attr:
            ...
            lat = torch.from_numpy(xr_dataset.latitude.values).float() + 180. # convert to non-negative values 
            ...
            self.edge_attr = torch.cat([self.lat_lon[self.edge_index[1]] - self.lat_lon[self.edge_index[0]], self.edge_dist], dim=-1)
...
class GraphResidualBlock(nn.Module):
    def __init__(self, input_size, hidden_size, out_size, heads=1, dropout=0, use_spectral_norm=True, use_edge_attr=True):
        super().__init__()
        ...
        self.transformer = TransformerConv(self.input_size, self.hidden_size // self.heads,
                                           heads=self.heads, dropout=self.dropout, concat=True, edge_dim=self.hidden_size if self.use_edge_attr else None)
...
if __name__ == "__main__":
    ...
    model =  model_class( 
                ...
                use_edge_attr=use_edge_attr).to("cpu")
