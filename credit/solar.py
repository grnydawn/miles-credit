...
def get_solar_radiation_loc(lon: float, lat: float, altitude: float,
                            ...
                            step_freq: str = "1h", sub_freq: str = "1Min"):
    """
    ...
    dates = pd.date_range(start=start_date_ts - pd.Timedelta(step_freq) + pd.Timedelta(sub_freq),
                          end=end_date_ts, freq=sub_freq, tz="utc")
    ...
    step_rad = trapezoid(np.reshape(solar_rad, (int(solar_rad.size // step_len), step_len)),
                         dx=sub_sec, axis=1)
    ...
    out_rad_da = xr.DataArray(step_rad.reshape(-1, 1, 1),
                              ...
                              attrs={"standard_name": "solar_irradiance",
                                     "long_name": "total solar irradiance", "units": "J m-2"})

    zenith_da = xr.DataArray(step_cos_zenith.values.reshape(-1, 1, 1),
                             coords={"time": step_dates, "latitude": [lat], "longitude": [lon]},
                             dims=("time", "latitude", "longitude"), name="solar_zenith_angle",
                              attrs={"standard_name": "cos_solar_zenith_angle",
                                     "long_name": "cosine of the solar zenith angle", "units": ""})
...
if __name__ == "__main__":
    ...
    for lon_val, lat_val in tqdm(zip(lon_grid.ravel(), lat_grid.ravel())):
        out = get_solar_radiation_loc(lon_val, lat_val, 0.0, "2016-01-01", "2016-12-31 23:00")
        solar_ts.append(out)
