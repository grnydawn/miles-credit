...
def test_full_state_pressure_interpolation():
    input_file = os.path.abspath("data/test_interp.nc")
    model_level_file = os.path.abspath("../credit/metadata/ERA5_Lev_Info.nc")
    ds = xr.open_dataset(input_file)
    model_levels = xr.open_dataset(model_level_file)
    pressure_levels = np.array([200.0, 500.0, 700.0, 850.0, 1000.0]) * 100.0
    model_a = model_levels["a_model"].loc[ds_sub["level"]].values
    model_b = model_levels["b_model"].loc[ds_sub["level"]].values
