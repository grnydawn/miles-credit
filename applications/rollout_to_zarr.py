...
def save_zarr_increment(y_pred, utc_datetime, latlons, conf, forecast_hour, nc_filename, verbose=False):
    """
    Save increment to a unique Zarr file using Dask for parallel processing.
    """
    # Call make_xarray initially
...
def merge_files(nc_filename_base, conf, verbose=False):
    """
    Merge all individual NetCDF files into one consolidated file, sorted by forecast_hour.
    """
    save_location = conf["predict"]["save_forecast"]
    file_pattern = os.path.join(save_location, f"pred_{nc_filename_base}_*.zarr")
    file_list = sorted(glob.glob(file_pattern), key=lambda x: int(x.split('_')[-1].split('.zarr')[0]))
    ...
    merged_filename = os.path.join(save_location, f"pred_{nc_filename_base}.zarr")
    ...
    with torch.no_grad():
        ...
        for k, batch in enumerate(data_loader):
            ...
            result = p.apply_async(
                save_zarr_increment,
                (y_pred, utc_datetime, latlons, conf, forecast_hour, init_datetime_str)
            )
            ...
            if batch["stop_forecast"][0]:
                ...
                merge_files(init_datetime_str, conf, verbose=True)
