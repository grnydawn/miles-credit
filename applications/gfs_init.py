...
os.makedirs(config['save_out_path'], exist_ok=True)
...
variables = config["upper_air_variables"] + config["surface_variables"]
date = pd.Timestamp(config["init_time"])
gfs_init = build_GFS_init(output_grid=credit_grid,
                          date=date,
                          variables=variables,
                          model_level_indices=model_level_indices)

gfs_init.to_zarr(join(config["save_out_path"], f"gfs_init_{date.strftime('%Y%m%d_%H00')}.zarr"))
