...
cluster = PBSCluster(account=project_num,walltime='12:00:00',cores=1, memory='70GB',shared_temp_directory='/glade/derecho/scratch/wchapman/tmp',queue='main')
...
def make_nc_files_optimized(files_dict, Dateswanted, Dayswanted, FPout, prefix_out,levels):
    """
    ...
    for dw in Dayswanted:
        ...
        DS_u = xr.open_mfdataset(smatch_u, parallel=True, preprocess=lambda ds: preprocess_levs(ds, levels))
        sel_times = Dateswanted.intersection(DS_u['time'])
        DS_v = xr.open_mfdataset(smatch_v, parallel=True, preprocess=lambda ds: preprocess_levs(ds, levels)).sel(time=sel_times)
        DS_t = xr.open_mfdataset(smatch_t, parallel=True, preprocess=lambda ds: preprocess_levs(ds, levels)).sel(time=sel_times)
        DS_q = xr.open_mfdataset(smatch_q, parallel=True, preprocess=lambda ds: preprocess_levs(ds, levels)).sel(time=sel_times)
        ...
        DS = DS.drop_vars(['weight','utc_date','a_half','zero','a_model','b_model','b_half'])
    ...
    if not check_file_size(AFout,size_limit):
        ...
        if len(Dayswantedtot)<4:
            ...
            for dd in divided_lists:
                ...
                #make the files:
                print('...starting processing...')
                delayed_writes,created_files = make_nc_files_optimized(files_dict, Dateswanted, Dayswanted,FPout, prefix_out, levels)
