...
def main(year):
    ...
    encoding = {var: {'chunks': tuple(chunks.values())} for var, chunks in chunk_sizes.items()}

    print('...writing dataset...')
    # Save the combined dataset back to the Zarr store with the specified encoding
    combined_ds.to_zarr(f'/glade/derecho/scratch/wchapman/TOTAL_{year}-01-01_{year}-12-31_staged.zarr', mode='w', encoding=encoding)

    print(f"Appended DSlev to DSbb for U, V, T, Q, rechunked, and saved the result back to the Zarr store for year {year}.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process Zarr datasets for a specific year.")
    parser.add_argument('year', type=int, help='Year for which to process the datasets, e.g., 2020')
