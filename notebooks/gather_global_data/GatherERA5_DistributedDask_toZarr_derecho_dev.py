...
FPout = '/glade/derecho/scratch/wchapman/STAGING2/' #where do you want the files stored?
...
def fp_dates_wanted(Dateswanted):
    ...
    for yrm_fp in np.unique(list_yrm):
        for dayday in np.unique(day_wanted):
            ...
            fp_z.append(sorted(glob.glob('/glade/campaign/collections/rda/data/ds633.0/e5.oper.an.pl/'+yrm_fp+'/'+'*_z*'+yrm_fp+f'{dayday:02}'+'*.nc')))#WEC
            fp_ub.append(sorted(glob.glob('/glade/campaign/collections/rda/data/ds633.0/e5.oper.an.pl/'+yrm_fp+'/'+'*_u.*'+yrm_fp+f'{dayday:02}'+'*.nc')))#WEC
            fp_vb.append(sorted(glob.glob('/glade/campaign/collections/rda/data/ds633.0/e5.oper.an.pl/'+yrm_fp+'/'+'*_v.*'+yrm_fp+f'{dayday:02}'+'*.nc')))#WEC
            fp_tb.append(sorted(glob.glob('/glade/campaign/collections/rda/data/ds633.0/e5.oper.an.pl/'+yrm_fp+'/'+'*_t.*'+yrm_fp+f'{dayday:02}'+'*.nc')))#WEC
            fp_qb.append(sorted(glob.glob('/glade/campaign/collections/rda/data/ds633.0/e5.oper.an.pl/'+yrm_fp+'/'+'*_q.*'+yrm_fp+f'{dayday:02}'+'*.nc')))#WEC
            if yrm_fp[:4]+'-'+yrm_fp[4:]+'-'+f'{dayday:02}' == lastday:
                break
        fp_t2m.append(sorted(glob.glob('/glade/campaign/collections/rda/data/ds633.0/e5.oper.an.sfc/'+yrm_fp+'/'+'*_2t*'+yrm_fp+'*.nc')))#WEC
    ...
    files_dict ={'u':np.unique(fp_u),'v':np.unique(fp_v),'t':np.unique(fp_t),'q':np.unique(fp_q),'ps':np.unique(fp_ps),'z':np.unique(fp_z),'ub':np.unique(fp_ub),'vb':np.unique(fp_vb),'tb':np.unique(fp_tb),'qb':np.unique(fp_qb),'t2m':np.unique(fp_t2m)}
...
def regrid_(file_list,varin,varout,method='bilinear',level=None):
    ...
    if level is None:
        ...
        DSog = xr.open_mfdataset(file_list,parallel=True).sel(level=level).squeeze().drop('level')
        
    ds_out = xr.open_dataset('/glade/u/home/wchapman/MLWPS/Stage_Data/ML_grid.nc')
    fn = xr.open_dataset('/glade/u/home/wchapman/MLWPS/Stage_Data/bilinear_721x1440_640x1280.nc')
...
def regrid_save_zarr(regrid_sl,varin_l,varout_l,lev_list,start_time,end_time):
    ...
    for ii in range(len(varin_l)):
        ...
        if os.path.exists(nameoutz):
            ...
            DSout = regrid_(regrid_sl[varin_l[ii]],varin=varin_l[ii],varout=varout_l[ii],level=lev_list[ii],method='bilinear') #WEC
...
if __name__ == '__main__':
    ...
    z_list = flatten_list(z_list)#WEC
    ub_list = flatten_list(ub_list)#WEC
    vb_list = flatten_list(vb_list)#WEC
    tb_list = flatten_list(tb_list)#WEC
    qb_list = flatten_list(qb_list)#WEC
    regrid_sl = {'VAR_2T':np.unique(t2m_list),'Z':np.unique(z_list),'U':np.unique(ub_list),'V':np.unique(vb_list),'T':np.unique(tb_list),'Q':np.unique(qb_list)}#WEC
    
    
    
    print('... begining to regrid please...!')
    start_time = time.time()  # Record the start time
    zarr_out = regrid_save_zarr(regrid_sl,varin_l,varout_l,lev_list,start_date,end_date_minus1)
    print(zarr_out)
    elapsed_time = time.time() - start_time
    print(f" regridding phase executed in {elapsed_time} seconds")
        
    DSstore = []
    for zz in zarr_out: 
        DS = xr.open_zarr(zz).sel(time=slice(start_date,end_date_minus1))
        DSstore.append(DS)
            
    DStot = xr.merge(DSstore)
    DStot.to_zarr(f"{FPout}/ONELEV_{start_date}_{end_date_minus1}_staged.zarr")
    ################## regridding + saving series: #WEC
    print('...done onelev...Merging All...')
    ### merge and Save
    DS1=xr.open_zarr(f"{FPout}/ONELEV_{start_date}_{end_date_minus1}_staged.zarr") 
    DS2=xr.open_zarr(f"{FPout}/AllFiles_{start_date}_{end_date_minus1}_staged.zarr")
    DSfin = xr.merge([DS1,DS2])
    DSfin.to_zarr(f"{FPout}/TOTAL_{start_date}_{end_date_minus1}_staged.zarr")
